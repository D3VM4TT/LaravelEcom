...
@section('content_title', 'User Management: Edit User')
        ...
        <form id="createUserForm" name="createUserForm" method="post" action="{{ route('admin.users.update', ["user" => $user]) }}" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full">
            ...
            <div class="mb-4">
                ...
                <input
                    ...
                    id="name" type="text" placeholder="Username" name="name" value="{{$user->name}}">
            ...
            <div class="mb-6">
                ...
                <input
                    ...
                    id="email" type="email" placeholder="johan@email.com" name="email" value="{{$user->email}}">
                @if ($errors->has('email'))
            ...
            <div class="mb-6">
                ...
                <select multiple id="roles" name="roles"
                    ...
                    @foreach ($roles as $role)
                        <option @if(in_array($role, $userRoles))selected @endif value="{{ $role }}">{{ $role }}</option>
                    @endforeach
                </select>
                @if ($errors->has('roles'))
