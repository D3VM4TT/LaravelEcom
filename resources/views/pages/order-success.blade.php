...
@section('content')
    ...
    <div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        ...
        <div class="flex justify-start item-start space-y-2 flex-col">
            ...
            <p class="text-base dark:text-gray-300 font-medium leading-6 text-gray-600">{{ \Carbon\Carbon::parse($order->created_at)->format('jS F Y \a\t h:i A') }}</p>
        </div>
                    ...
                    @foreach($order->items as $item)
                        <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                            <div class="pb-4 md:pb-8 w-full md:w-40">
                                <img class="w-full hidden md:block" src="{{asset('/img/products/' . $item->product->image)}}" alt="dress" />
                                <img class="w-full md:hidden" src="{{asset('/img/products/' . $item->product->image)}}" alt="dress" />
                            </div>
                            <div class="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                                <div class="w-full flex flex-col justify-start items-start space-y-8">
                                    ...
                                    <div class="flex justify-start items-start flex-col space-y-2">
                                        ...
                                        <p class="text-sm dark:text-white leading-none text-gray-800"><span class="dark:text-gray-400 text-gray-300">Color: </span> {{$item->color->name}}</p>
                                    </div>
                                </div>
                                <div class="flex justify-between space-x-8 items-start w-full">
                                    <p class="text-base dark:text-white xl:text-lg leading-6">${{$item->product->price / 100}}</p>
                                    <p class="text-base dark:text-white xl:text-lg leading-6 text-gray-800">{{$item->quantity}}</p>
                                    <p class="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">${{$item->total / 100}}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div class="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
                        ...
                        <div class="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                            <div class="flex justify-between w-full">
                                ...
                                <p class="text-base dark:text-gray-300 leading-4 text-gray-600">${{$order->total / 100}}</p>
                            </div>
                    ...
                    <div class="flex flex-col justify-start items-start flex-shrink-0">
                        <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                            ...
                            <div class="flex justify-start items-start flex-col space-y-2">
                                ...
                                <p class="text-sm dark:text-gray-300 leading-5 text-gray-600">{{count($order->user->orders) - 1  }} Previous Orders</p>
                            </div>
                        </div>
...
{{--                            </div>--}}
                            <div class="mt-7 flex justify-center md:justify-start items-center md:items-start flex-col space-y-4">
